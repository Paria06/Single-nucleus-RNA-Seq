...
print("DEG_med_cluster")
# all the results, table, figuures of this analysis are saved in the below directory
outDir <- "/home/paria/scratch/als-project/analysis/ocu_med_sc/rpca/pseudo_bulk_DEG/als_vs_con_each_region/DEG_ALS_vs_con_med/cluster"
...
dds$region <- relevel(dds$region, ref = "med")
...
res <- results(dds, contrast=list(c("group_id_als_vs_con")))
res <- lfcShrink(dds, contrast =  contrast, res =res, type="ashr")
res$cluster_id <- clusters[i]
res_list[[i]] <- res
...
# Combine the results from all iterations into a single object
res_list <- Filter(Negate(is.null), res_list)
res_combined <- do.call(rbind, res_list)

# Save the combined results to an RDS file
saveRDS(object = res_combined, file = file.path(outDir, "res.rds"))
